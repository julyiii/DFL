...
class Graph():
    ...
    def get_distance_to_center(self,layout): 
        ...
        elif layout == 'interhand_gt':
            dist_center_r = [4,3,2,1,4,3,2,1,4,3,2,1,4,3,2,1,4,3,2,1,0]
            dist_center_l = [i+1 for i in  dist_center_r]
            dist_center = dist_center_r + dist_center_l
        elif layout == 'interhand_gt_bone':
            dist_center_r = [5,4,3,2,4,3,2,1,3,2,1,0,4,3,2,1,5,4,3,2]
            dist_center_l = [i+1 for i in  dist_center_r]
            dist_center = dist_center_r + dist_center_l
        return dist_center
    ...
    def get_edge(self, layout):
        ...
        if layout == 'hm36_gt':
            ...
            self.num_node_each = 42

            neighbour_base_r = [(0, 1), (1, 2), (3,2), (3, 20), 
                              (4,5), (6, 5), (6, 7), (7,20), 
                              (9, 8), (10,9), (10, 11),(11,20),
                              (12,13),(13,14),(14,15),(15,20),
                              (16,17),(17,18),(18,19),(19,20)
                              ]
            neighbour_base_l = [(i+self.num_node_each//2,j+self.num_node_each//2)  for (i,j) in neighbour_base_r]

            neighbour_base = neighbour_base_r + neighbour_base_l
                        
            sym_base = [(i,i+21) for  i in range(21)]  
           ...
           # self.edge = self_link + self.neighbour_link_all + self.sym_link_all + time_link # len=39
            ...
            self.num_node_each = 20*2

            neighbour_base_r = [[0, 1], [1, 2], [2, 3],
                                [4,5], [5, 6], [6, 7],
                                [8,9],[9,10],[10,11],
                                [12,13],[13,14],[14,15],
                                [16,17],[17,18],[18,19]
            ...
            sym_base = [(i,i+20) for  i in range(20)]  
                ...
                for i in range(self.num_node):
                    for j in range(self.num_node):
                        if self.hop_dis[j, i] == hop: # 0:diagonal; 1:adjacent point
                            ...
                            elif self.dist_center[j] == self.dist_center[i]:  #到root的距离等距离 0hop时，可以进入if判断的语句一定i=j，故此条件一定此成立。故a_root = 归一化的邻接矩阵的对角线值。1hop时，意味着某节点的1hop的那些邻接节点距离根节点距离相同的节点的归一化邻接矩阵的值
                                a_root[j, i] = normalize_adjacency[j, i]
                            elif self.dist_center[j] > self.dist_center[i]:   #hop > 0 时，j比i到root的距离近
...
def get_hop_distance(num_node, edge, max_hop=1): # 建立邻接矩阵,相邻则置0   
    ...
    for d in range(max_hop, -1, -1): #   A(i,j) = 1 while A(i,i) = 0  相邻为1 对角为0
