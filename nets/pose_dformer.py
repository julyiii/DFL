...
class DeformableBlock(nn.Module):
    ...
    def forward(self, x, ref, features_list):
        ...
        b, l, p, c = x.shape
        residual = x
        x = self.norm1(x + x_0)  #注意每一层根据ref和ref处的特征 单独估计偏移和权重
...
class PoseTransformer(nn.Module):
    def __init__(self, config=None, num_frame=1, num_joints=42, in_chans=256,
        ...
        #     nn.Linear(in_chans, embed_dim_ratio),
        #     nn.Linear(in_chans, embed_dim_ratio),
        #     nn.Linear(in_chans, embed_dim_ratio),
        #     nn.Linear(in_chans, embed_dim_ratio),
        #     ])
        ...
        self.context_blocks = nn.ModuleList([
            DeformableBlock(dim=embed_dim_ratio, num_heads=4, num_samples=16, qkv_bias=qkv_bias, drop_path=dpr[i])
            for i in range(depth)])
    ...
    def forward(self, keypoints_2d, ref, features_list):
        ...
        x = x[:,-1]
        x = x.transpose(1,0)
        #x = rearrange(x, 'b l p c -> (b p) l c')    #b:bs l:layer_num p:point_num c:feature dim 
