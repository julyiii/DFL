...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, img, mask):
        ...
        pos = pos.permute(0, 3, 1, 2) # N x hidden_dim x H x W

        # pos = torch.stack((pos_y, pos_x), dim=-2).flatten(-2)
        # pos = pos.permute(0, 3, 1, 2) # N x hidden_dim x H x W
        ...
        if vis:
            ...
            P = pos_x[0][0].cpu().numpy()
            cat = plt.matshow(P)
            plt.gcf().colorbar(cat)
            plt.show()
            plt.savefig('pos_embeding.png', dpi=100)
...
def build_position_encoding(args):
    ...
    if args.position_embedding in ('v2', 'sine'):
        ...
        # position_embedding = PositionEmbeddingSimpleCat(N_steps)
